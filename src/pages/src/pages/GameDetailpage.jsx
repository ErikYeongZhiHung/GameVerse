    ...
    return {
      minimum: {
        ...
        graphics: isHighEnd
          ...
          : "NVIDIA GTX 750 Ti / AMD Radeon R7 260X",
        directX: "Version 12",
  ...
  return (
    <div className="mb-8">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <FaApple
          className="text-2xl text-gray-400 opacity-50"
          title="Mac (Coming Soon)"
        />
        <FaLinux
          className="text-2xl text-orange-500 opacity-50"
          title="Linux (Coming Soon)"
        />
      </div>
        ...
        <div className="grid gap-4">
          {Object.entries(requirements[activeTab]).map(([key, value]) => (
            <div key={key} className="flex">
              <div className="w-24 text-gray-400 capitalize font-medium">
                {key}:
              </div>
...
const StatisticsAnalysis = ({ gameName, gameData }) => {
  ...
  const generateStats = (gameName, gameData) => {
    const isPopular =
      gameName?.toLowerCase().includes("grand theft auto") ||
      gameName?.toLowerCase().includes("witcher") ||
      gameName?.toLowerCase().includes("cyberpunk") ||
      gameName?.toLowerCase().includes("call of duty");
    ...
    return {
      overview: {
        ...
        activeToday: Math.floor(
          baseUsers * 0.1 + Math.random() * (baseUsers * 0.05)
        ),
        peakToday: Math.floor(
          baseUsers * 0.15 + Math.random() * (baseUsers * 0.1)
        ),
      ...
      performance: {
        avgPlaytime: isPopular
          ? `${25 + Math.floor(Math.random() * 50)} hours`
          : `${8 + Math.floor(Math.random() * 20)} hours`,
      ...
      demographics: {
        regions: [
          ...
          {
            name: "Europe",
            percentage: 28 + Math.floor(Math.random() * 8),
            flag: "🇪🇺",
          },
          {
            name: "Asia",
          ...
          {
            name: "Other",
            percentage: 12 + Math.floor(Math.random() * 5),
            flag: "🌍",
          },
        ],
        ageGroups: [
...
const UserReviews = ({ gameName }) => {
  ...
  const generateReviews = (gameName) => {
    ...
    const isPopular =
      gameType?.includes("grand theft auto") ||
  ...
  const recommendationPercent = Math.round(
    (reviews.filter((r) => r.recommended).length / reviews.length) * 100
  );
...
export default function GameDetailSection() {
  ...
  const handleConfirmPaymentMethod = (method) => {
    // Only proceed for Credit Card and Debit Card
  ...
  const handlePaymentDetailsSubmit = (paymentData) => {
    ...
    buyNowMutation.mutate(paymentData);
  };
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="w-full lg:w-[360px] bg-[#18181b] rounded-2xl p-6 shadow-lg">
          ...
          <div className="relative mb-3">
            <button
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-70"
              onClick={() => {
                if (!user) {
                  navigate("/signin");
                  return;
            ...
            {!user && showBuyNowTooltip && (
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 bg-black text-white text-xs px-3 py-2 rounded shadow-lg z-10 whitespace-nowrap">
                Click to login and purchase games
              </div>
            )}
          </div>
          ...
          {showPaymentDetailsModal && (
            <PaymentDetailsModal
              ...
              gameData={{
                name: game.name,
                price:
                  game?.price_overview?.final_formatted?.replace(
                    /[^0-9.]/g,
                    ""
                  ) || 0,
              }}
          ...
          <div className="relative">
            <button
              className="w-full bg-[#232329] hover:bg-[#2e2e33] text-white font-semibold py-3 rounded-lg transition disabled:opacity-70"
              onClick={() => {
                if (!user) {
                  navigate("/signin");
                  return;
            ...
            {!user && showTooltip && (
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 bg-black text-white text-xs px-3 py-2 rounded shadow-lg z-10 whitespace-nowrap">
                Click to login and add to cart
              </div>
            )}
          </div>
        </div>
